...
def ex2():
    ...
    loda.fit(train_data)
    scores_loda = loda.decision_function(test_data)
    ax[0].scatter(test_data[:, 0], test_data[:, 1], c=scores_loda)
    ax[0].set_title("Bins=10")
    loda = LODA(contamination=0.02, n_bins=50)
    loda.fit(train_data)
    scores_loda = loda.decision_function(test_data)
    ax[1].scatter(test_data[:, 0], test_data[:, 1], c=scores_loda)
    ax[1].set_title("Bins=50")
    loda = LODA(contamination=0.02, n_bins=200)
    loda.fit(train_data)
    scores_loda = loda.decision_function(test_data)
    ax[2].scatter(test_data[:, 0], test_data[:, 1], c=scores_loda)
    ...
    fig, ax = plt.subplots(1, 3, figsize=(20, 8))
    dif = DIF(contamination=0.02, hidden_neurons=[16])
    dif.fit(train_data)
    scores_dif = dif.decision_function(test_data)
    ...
    ax[0].scatter(*test_data.T, c=scores_iforest)
    ax[0].set_title("IForest")
    ax[1].scatter(*test_data.T, c=scores_dif)
    ax[1].set_title("DIF")
    ...
    loda.fit(train_data)
    scores_loda = loda.decision_function(test_data)
    ax[0].scatter(*test_data.T, c=scores_loda)
    ax[0].set_title("Bins=10")
    loda = LODA(contamination=0.02, n_bins=50)
    loda.fit(train_data)
    scores_loda = loda.decision_function(test_data)
    ax[1].scatter(*test_data.T, c=scores_loda)
    ax[1].set_title("Bins=50")
    loda = LODA(contamination=0.02, n_bins=200)
    loda.fit(train_data)
    scores_loda = loda.decision_function(test_data)
    ...
    dif.fit(train_data)
    scores_dif = dif.decision_function(test_data)
    ax[0].scatter(*test_data.T, c=scores_dif)
    ax[0].set_title("Bins=10")
    dif = DIF(contamination=0.02, hidden_neurons=[32, 16])
    dif.fit(train_data)
    scores_dif = dif.decision_function(test_data)
